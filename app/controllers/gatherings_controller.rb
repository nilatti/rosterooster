class GatheringsController < ApplicationController
  ...
  def create
    @event = Event.find(params[:event_id])
    @gathering = @event.gatherings.build(gathering_params)

    respond_to do |format|
      if @gathering.save
        format.html { redirect_to @gathering.event, notice: 'Gathering was successfully created.' }
  ...
  private
    ...
    def gathering_params
      params.require(:gathering).permit(:location, :start, :end, :leader, :name, :event_id)
    end
end
